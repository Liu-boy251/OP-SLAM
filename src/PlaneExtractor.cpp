...
bool PlaneDetection::readDepthImage(cv::Mat depthImg, cv::Mat &K)
{
	...
	if(depthImg.empty())
	{
			cout << "depth_img is empty!!" << endl;
			return false;
	}
	cout << "depth type:  " << depthImg.depth() <<  endl;

	if(depthImg.depth() == CV_32FC1 || depthImg.depth() == CV_32F)
	{
        depthImg.convertTo(depth_img,CV_16U,1000);
		cout << "depth type:  " << depth_img.depth() << endl;
	}
	else
	{
		depth_img = depthImg;
