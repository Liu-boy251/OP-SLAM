        ...
        mbDeactivateLocalizationMode(false)
{
    ...
    mptLocalMapping = new thread(&OP_SLAM::LocalMapping::Run, mpLocalMapper);

    //Initialize the Loop Closing thread and launch
    mpLoopCloser = new LoopClosing(mpMap, mpKeyFrameDatabase, mpVocabulary, mSensor!=MONOCULAR);
    mptLoopClosing = new thread(&OP_SLAM::LoopClosing::Run, mpLoopCloser);
...
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp)
{
    ...
    //图像裁剪 2023.7.26 
    //cv::Rect rect(0,40,640,400);
    //const cv::Mat &img = im(rect).clone();
...
void System::SaveTrajectoryTUM(const string &filename)
{
    ...
    list<OP_SLAM::KeyFrame*>::iterator lRit = mpTracker->mlpReferences.begin();
