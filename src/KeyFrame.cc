...
//    cout << "Frame::lineDescriptorMAD——matches_nn = "<<matches_nn.size() << endl;
    ...
    void KeyFrame::AddMapPlane(OP_SLAM::MapPlane *pMP, const int &idx) {
        unique_lock<mutex> lock(mMutexFeatures);
        mvpMapPlanes[idx] = pMP;
    }

    void KeyFrame::AddMapVerticalPlane(OP_SLAM::MapPlane *pMP, const int &idx) {
        unique_lock<mutex> lock(mMutexFeatures);
        mvpVerticalPlanes[idx] = pMP;
    }

    void KeyFrame::AddMapParallelPlane(OP_SLAM::MapPlane *pMP, const int &idx) {
        unique_lock<mutex> lock(mMutexFeatures);
