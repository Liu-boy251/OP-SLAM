...
namespace OP_SLAM {
    ...
    Frame::Frame(const cv::Mat &imRGB, const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp,
        ...
        thread threadLines(&OP_SLAM::Frame::ExtractLSD, this, imGray, depth, tmpK);
        thread threadPoints(&OP_SLAM::Frame::ExtractORB, this, 0, imGray);
        thread threadPlanes(&OP_SLAM::Frame::ComputePlanes, this, depth, imDepth, imRGB, K);
        threadPoints.join();
        threadLines.join();
        threadPlanes.join();
    ...
    void Frame::ComputePlanes(const cv::Mat &imDepth, const cv::Mat &Depth, const cv::Mat &imRGB, cv::Mat K) {
        ...
        {
            PCL_ERROR ("Could not estimate a OP model for the given initial plane.\n");
            return;
        }
